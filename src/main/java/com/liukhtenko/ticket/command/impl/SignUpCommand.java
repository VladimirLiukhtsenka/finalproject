    ...
    public String execute(HttpServletRequest request) {
        ...
        if (!FormValidator.isPost(request)) {
            ...
            try {
                ...
                String phone = request.getParameter(FormParameterName.FORM_PARAM_PHONE);
                if (FormValidator.isValidString(phone, FormRegexValidator.PHONE)) { // FIXME: 02.02.2020 upgrade validate
                    user.setPhone(phone);
                }
                String name = request.getParameter(FormParameterName.FORM_PARAM_NAME);
                if (FormValidator.isValidString(name, FormRegexValidator.LOGIN)) {
                    user.setName(name);
                }
                String surName = request.getParameter(FormParameterName.FORM_PARAM_SURNAME);
                if (FormValidator.isValidString(surName, FormRegexValidator.LOGIN)) {
                    user.setSurName(surName);
                }
                String fatherName = request.getParameter(FormParameterName.FORM_PARAM_FATHER_NAME);
                if (FormValidator.isValidString(fatherName, FormRegexValidator.LOGIN)) {
                    user.setFatherName(fatherName);
                }
                String gender = request.getParameter(FormParameterName.FORM_PARAM_GENDER);
                if (FormValidator.isValidNumber(gender)) {
                    user.setGender(Byte.parseByte(gender));
                }
                String password = request.getParameter(FormParameterName.FORM_PARAM_PASSWORD);
                if (FormValidator.isValidString(password, FormRegexValidator.PASSWORD)) {
                    user.setPassword(password);
                }
                String mail = request.getParameter(FormParameterName.FORM_PARAM_MAIL);
