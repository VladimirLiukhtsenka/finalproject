...
<%@ include file="include/menu.jsp" %>
    <h1 align="center"><fmt:message key="navBar.movie"/></h1>
    <div class="row">
    ...
    <table style = "margin: auto;padding: 0;">
            <tr>
                <c:if test="${currentPage != 1}">
                    <td><a href="do?command=View_for_movie_event&page=${currentPage - 1}"><fmt:message key="button.previous"/></a></td>
                </c:if>
                <c:forEach begin="1" end="${noOfPages}" var="i">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <td style = "padding: 6px;"><a href="do?command=View_for_movie_event&page=${i}">${i}</a></td>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
                <c:if test="${currentPage < noOfPages}">
                    <td><a href="do?command=View_for_movie_event&page=${currentPage + 1}"><fmt:message key="button.next"/></a></td>
                </c:if>
            </tr>
    </table>
</body>
